...
const LandingPage = ({ setView }) => (
  <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center font-sans">
    <div className="max-w-4xl">
      ...
      <p className="text-slate-500 mb-12 text-2xl font-black tracking-tight">Master The Code, Charm The Future.</p>
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[100] animate-in fade-in duration-300 font-sans text-left">
        <div className="bg-white p-10 rounded-[3rem] w-full max-w-md shadow-2xl border border-slate-50 relative">
            ...
            <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 space-y-4">
                ...
                <p className="text-xs font-bold text-slate-700">Unlock this platform for <span className="text-indigo-600 font-bold">K30 / Month</span>.</p>
                <div className="pt-2 space-y-1">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Payment Details</p>
                        ...
                        <div className="p-4 bg-indigo-950/40 border-b border-indigo-900 shrink-0">
                            {tab === 'video' ? (
                                <div className="flex gap-2 mb-2">
                                    <button onClick={async ()=>{const t=prompt("Title:"); const u=prompt("YouTube URL:"); if(t&&u){await axios.post(API + '/videos/link',{courseId,title:t,url:u});load();}}} className="flex-1 bg-indigo-600 text-white py-2 rounded text-[10px] font-black uppercase tracking-tighter shadow-sm">ADD LINK</button>
                                    <label className="flex-1 bg-slate-800 py-2 rounded text-[10px] font-black text-center cursor-pointer uppercase tracking-tighter shadow-sm text-center">UPLOAD <input type="file" hidden onChange={(e)=>uploadVideo(e.target.files[0])}/></label>
                                </div>
                            ) : (
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-left">
            <nav className="p-6 bg-white border-b flex justify-between items-center sticky top-0 z-10 shrink-0 shadow-sm text-left">
                <div className="font-bold text-indigo-700 flex items-center gap-2 text-2xl tracking-tighter leading-none">Hi, {user.name}</div>
            ...
            <div className="p-8 md:p-16 max-w-6xl mx-auto flex-1 w-full text-left">
                {activeTab === 'platforms' ? (
                    <>
                        <h2 className="text-4xl font-black mb-12 text-slate-900 tracking-tight uppercase tracking-tighter">Platforms</h2>
                                ...
                                <div key={c.id} onClick={()=>onJoinCourse(c.id)} className="group relative bg-white border border-slate-200 p-10 rounded-[3.5rem] cursor-pointer hover:shadow-2xl transition-all hover:-translate-y-2 text-left">
                                    ...
                                    {user.role === 'manager' && (
                                        <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-all text-left">
                                            <button onClick={async (e)=>{e.stopPropagation(); await axios.put(API + "/courses/reorder/swap", {courseId: c.id, direction: "up"}); load();}} className="p-2 bg-slate-50 rounded-full hover:text-indigo-600"><ArrowUp size={16}/></button>
                                            <button onClick={async (e)=>{e.stopPropagation(); await axios.put(API + "/courses/reorder/swap", {courseId: c.id, direction: "down"}); load();}} className="p-2 bg-slate-50 rounded-full hover:text-indigo-600"><ArrowDown size={16}/></button>
...
export default function App() {
    ...
    useEffect(() => {
        ...
        if (t) { 
            ...
            axios.get(API + '/auth/me').then(res => { setUser(res.data); setView('dashboard'); }).catch(()=>{localStorage.clear(); setView('landing');}); 
        }
    }, []);
    ...
    const handleAuth = async (e) => {
        ...
        try { 
            ...
            if(auth.isLogin) {
                ...
                alert("Account Created! Please Login."); setAuth({...auth, isLogin: true});
            }
        ...
        <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center font-sans text-left text-left">
            {view === 'landing' ? <LandingPage setView={setView}/> : (
                <form onSubmit={handleAuth} className="bg-white p-12 rounded-[4.5rem] shadow-2xl w-full max-w-sm border border-slate-100 animate-in zoom-in-95 duration-500">
                    <h2 className="text-3xl font-black mb-10 text-center tracking-tighter uppercase">Welcome v1.4.</h2>
