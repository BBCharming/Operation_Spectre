...
const app = express();
const PORT = 4000;
const JWT_SECRET = 'charming-secret-2025';
...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        db.prepare('INSERT INTO users (id, email, password, name, role) VALUES (?, ?, ?, ?, ?)').run(nanoid(), email, hashed, name, 'student');
        const token = jwt.sign({ id: nanoid(), email, role: 'student', name }, JWT_SECRET);
...
app.put('/api/courses/:id', authenticate, (req, res) => {
    console.log("📝 Renaming Course:", req.params.id, "to", req.body.title);
    db.prepare('UPDATE courses SET title = ? WHERE id = ?').run(req.body.title, req.params.id);
...
const upload = multer({ storage: multer.diskStorage({ destination: uploadPath, filename: (req, file, cb) => cb(null, `${Date.now()}-${file.originalname}`) }) });
app.post('/api/upload', authenticate, upload.single('file'), (req, res) => {
    const max = db.prepare('SELECT MAX(order_idx) as m FROM videos WHERE course_id = ?').get(req.body.courseId).m || 0;
    db.prepare("INSERT INTO videos (id, course_id, title, filename, path, order_idx) VALUES (?, ?, ?, ?, ?, ?)").run(
        nanoid(), req.body.courseId, req.body.title, req.file.filename, `/uploads/${req.file.filename}`, max + 1
    );
    res.json({ success: true });
});
...
app.put('/api/videos/reorder', authenticate, (req, res) => {
    ...
    if(sib) {
        console.log("↕️ Swapping", cur.title, "with", sib.title);
...
app.get('/api/users', authenticate, (req, res) => {
    res.json(db.prepare("SELECT id, name, email, subscription_status FROM users WHERE role = 'student'").all());
});
...
app.listen(PORT, () => console.log(`🚀 Executioner Server on ${PORT}`));
