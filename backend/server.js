...
app.get('/api/courses', authenticate, (req, res) => {
    ...
    res.json(data.map(c => ({
        ...c, 
        videoCount: db.prepare('SELECT COUNT(*) as cnt FROM videos WHERE course_id = ?').get(c.id).cnt || 0 
    })));
});
...
app.put('/api/courses/reorder/swap', authenticate, (req, res) => {
    ...
    if(sib) {
        db.transaction(() => {
            db.prepare('UPDATE courses SET order_idx = ? WHERE id = ?').run(sib.order_idx, cur.id);
            db.prepare('UPDATE courses SET order_idx = ? WHERE id = ?').run(cur.order_idx, sib.id);
        })();
    }
    res.json({ success: true });
});
...
app.put('/api/videos/reorder', authenticate, (req, res) => {
    ...
    if(sib) { db.transaction(() => { db.prepare('UPDATE videos SET order_idx = ? WHERE id = ?').run(sib.order_idx, cur.id); db.prepare('UPDATE videos SET order_idx = ? WHERE id = ?').run(cur.order_idx, sib.id); })(); }
    res.json({ success: true });
});
...
app.post('/api/compile', authenticate, (req, res) => {
    ...
    exec(cmd, { timeout: 5000 }, (err, stdout, stderr) => {
        const clean = (stdout || stderr || err?.message || "Unknown Error").replace(new RegExp(process.cwd(), 'g'), "[project]");
        fs.remove(workDir).catch(()=>{});
        res.json({ output: clean });
    });
});

app.listen(4000, () => console.log('🚀 Engine v1.6 Sovereign Active'));
