...
app.put('/api/courses/reorder/swap', authenticate, (req, res) => {
    ...
    if(sib) {
        db.transaction(() => {
            const curIdx = cur.order_idx; const sibIdx = sib.order_idx;
            db.prepare('UPDATE courses SET order_idx = -1 WHERE id = ?').run(cur.id);
            db.prepare('UPDATE courses SET order_idx = ? WHERE id = ?').run(curIdx, sib.id);
            db.prepare('UPDATE courses SET order_idx = ? WHERE id = ?').run(sibIdx, cur.id);
        })();
    }
    res.json({ success: true });
});
...
app.put('/api/videos/reorder', authenticate, (req, res) => {
    ...
    if(sib) {
        db.transaction(() => {
            ...
            db.prepare('UPDATE videos SET order_idx = ? WHERE id = ?').run(curIdx, sib.id);
            db.prepare('UPDATE videos SET order_idx = ? WHERE id = ?').run(sibIdx, cur.id);
        })();
    }
    res.json({ success: true });
});
...
app.post('/api/compile', authenticate, (req, res) => {
    ...
    exec(cmd, { timeout: 5000 }, (err, stdout, stderr) => {
        const clean = (stdout || stderr || "Execution successful.").replace(new RegExp(process.cwd(), 'g'), "[project]");
        fs.remove(workDir).catch(()=>{});
        res.json({ output: clean });
    });
});

app.listen(4000, () => console.log('🚀 Final Sovereign Engine v3.0 Active'));
